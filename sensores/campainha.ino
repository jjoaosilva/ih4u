...
#define TOPICO_SUBSCRIBE "ih4f/sensores/sub/bell"     //tópico MQTT de escuta
#define TOPICO_PUBLISH "ih4f/sensores/pub/bell"    //tópico MQTT de envio de informações para Broker
...
void mqtt_callback(char* topic, byte* payload, unsigned int length) 
{
    ...
    // ALTERNA ENTRE OS ESTADOS DE HIGH E LOW
//    if (estadoSaidaRed == 0) {
//      digitalWrite(D1, HIGH);
//      estadoSaidaRed = 1;
//      delay(5000);
...
void EnviaDadosParaBroker() {
  if(digitalRead(D8) == 0){ //SE A LEITURA DO PINO FOR IGUAL A LOW, FAZ
//    digitalWrite(D1, HIGH); //ACENDE O LED
//    MQTT.publish(TOPICO_PUBLISH, "ligado");
   }else{ //SENÃO, FAZ
    digitalWrite(D1, LOW); //APAGA O LED
     MQTT.publish(TOPICO_PUBLISH, "tem gente na porta");
   }
}
